...
def build_commands(results_path: Path, extra_args=None):
    """Build the list of commands for a given results directory,
    optionally appending extra CLI arguments (e.g. ['-d', '1'])."""
...
commands_euclidian = build_commands(results_path_euclidien)

# Same run, but with -d 1 added
results_path_astar = root_path / "demos_results_astar"
commands_astar = build_commands(results_path_astar, extra_args=["-d", "1"])

all_commands = commands_euclidian + commands_astar
...
with ThreadPoolExecutor(max_workers=MAX_PARALLEL) as executor:
    for cmd, returncode in executor.map(run_cmd, all_commands):
