 ...
 * @see Paths_t
 */
void cageWriteMol2(char *output, Cage_t *s, Paths_t *paths, int *interTree, int nb_atoms_paths, double cumul_RMSD) {
...
void cageWriteMol2_Spring(char *output, SpringPath_t* sp,time_t start, int collision, Options_t options){
  ...
  filestream = fopen(output, "w");
  if (filestream == NULL) {
    printf("The file %s could not be open for writing.\n", output);
    if (index)
      free(index);
    return;
  }
  // Entete Writting
  ret = fprintf(filestream, "# Molecule generated by CageSpringPath\n");
  ret = fprintf(filestream, "RMSD_dist : %f \nRMSD_angle : %f \n",sp->RMSD_dist,sp->RMSD_angle);
  ret = fprintf(filestream, "Time : %f\n", (double)(end-start)/60);
  ret = fprintf(filestream, "Collision : %d\n", collision);
  ret = fprintf(filestream, "@<TRIPOS>MOLECULE\n*****\n");
  ret = fprintf(filestream, " %d %d 0 0 0\n", cageNbAtom(s),cageNbEdges(s)); // Atom in the cage
  ret = fprintf(filestream, "SMALL\nGASTEIGER\n\n");
...
void writeCageOutput(Cage_t *cage, Paths_t *paths, int *interTree, Options_t options) {
  ...
  cumul_RMSD /= paths->numPaths; // divide by the number of paths to have the average RMSD

  if (!stats_only) {
    sprintf(outputname, "%s/%s_mot%d.mol2", under_dir_name, name, cpt_result_cage_egv);
    cageWriteMol2(outputname, cage, paths, interTree, nb_atoms_paths, cumul_RMSD);
  } else {
    recordResultMetrics(cumul_RMSD, min_path_length, max_path_length, total_path_length, path_length_samples,
                       nb_atoms_paths);
  }
